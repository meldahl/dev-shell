...
| A completion list with a header and aligned columns | `_dev_projects`: `compadd -Q -l -X header -d displays -a names`; width starts at the header's length | `zsh/dev-shell.zsh` |
| Anything in the line editor under zsh-autocomplete | the `_dev_has_autocomplete` branch: `_dev_menu_complete` (Tab), `_dev_list_select` (Down), `_dev_history_menu` (Up), `_dev_menu_enter run\|accept`, `_dev_history_colours` (comppostfuncs) | `zsh/dev-shell.zsh` "Shell UX"; contract in [docs/history-list-behavior.md](docs/history-list-behavior.md) |
...
- zsh-autocomplete couplings, each commented in the Shell UX block: history mode via `default-context`; Tab = `list-choices` + `menu-select -w` with a local `curcontext`; Enter decided by the widget that opens a menu (menuselect cannot run a user widget); colours via a `comppostfuncs` post-function rewriting `ZLS_COLORS`, re-registered at every precmd; the plugin's recent-dirs data directory created. After a plugin update, run `/verify` — the suite exercises every coupling.
