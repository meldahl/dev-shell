...
# Explorer integration only makes sense inside WSL.
_dev_is_wsl() { [[ -n $WSL_DISTRO_NAME ]] }

dev() {
  local open_code=0 open_explorer=0

  while [[ $1 == -* ]]; do
    case $1 in
      -c|--code)     open_code=1 ;;
      -e|--explorer) open_explorer=1 ;;
      -h|--help)
        print "usage: dev [-c|--code] [-e|--explorer] [project]"
        print "  -c, --code      open the project in VS Code"
        print "  -e, --explorer  open the project in Windows Explorer (WSL only)"
  ...
  if (( open_explorer )); then
    if ! _dev_is_wsl; then
      print -u2 "dev: --explorer requires WSL"
      return 1
    fi
